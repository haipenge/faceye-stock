	...
	<div class="row" style="margin-top: 0px; margin-bottom: 0px;">
		<div class="col-sm-8">
			<h2>
				...
				<a href="<c:url value="/stock/stock/home?trace=star"/>"
					class="btn btn-success">星标追踪</a>
			</h2>
		</div>
		<div class="col-sm-4 text-right">
			<a class="btn btn-primary btn-sm"
				href="<c:url value="/stock/stock/input"/>"> <fmt:message
					key="stock.stock.add"></fmt:message></a>
			<button class="btn btn-sm btn-warning" type="button"
				id="init-stock-category">初始化股票分类</button>
...
<div class="cl-mcont">
	<div class="block-flat">
		...
		<div class="row" style="margin-top: 0px; margin-bottom: 0px;">
			<div class="col-sm-8">
				<div class="content">
					<form action="<c:url value="/stock/stock/home"/>" method="post"
						role="form" class="form-inline" style="margin-bottom: 0px;" id="query-form">
						<fieldset>
								...
								<select name="EQ|category.$id" class="form-control input-sm">
									...
									<c:forEach var="category" items="${categories}">
										<option value="${category.id}"
											${searchParams.categoryid  eq category.id? "selected":""}>${category.name }</option>
									</c:forEach>
								</select>
							</div>
							...
							<div class="form-group">
								<input type="text" name="like|name" value="${searchParams.name}"
									placeholder="多个名字间使用逗号分隔" class="form-control input-sm">
							</div>
							<div class="form-group">
								<input type="text" name="like|code" value="${searchParams.code}"
									placeholder="多个股票代码间使用逗号分隔" class="form-control input-sm">
							</div>
							<div class="form-group">
								<input type="text" name="GTE|dailyStat.pe"
									value="${searchParams.minPe}" style="width: 90px;"
									placeholder="PE起始值" class="form-control input-sm">-<input
									type="text" name="LTE|dailyStat.pe"
									value="${searchParams.maxPe}" style="width: 90px;"
									placeholder="PE最大值" class="form-control input-sm">
							</div>
							<div class="form-group">
							...
							<div class="form-group">
								<button type="submit" value="query" class="btn btn-sm btn-primary">
									<fmt:message key="global.search"></fmt:message>
								</button>
								<button type="button" value="export" id="export" class="btn btn-sm btn-primary">
									导出
								</button>
							</div>
						</fieldset>
		...
		<div class="content category-container" style="display: none;">
			<c:forEach items="${categories}" var="category" varStatus="status">
				...
				<span class="label label-info"
					style="margin-top: 15px; margin-bottom: 5px; margin-left: 5px; margin-right: 5p;; width: 50px;"><a
		...
		<div class="content">
			<button type="button" class="btn btm-sm btn-primary"
				id="btn-multi-stock-compare">多股比对</button>
		</div>
		<div class="content">
						...
						<c:forEach items="${page.content}" var="stock">
							<tr>
								<td><input type="checkbox" name="check-single"
									value="${stock.id}"></td>
								<td>${stock.name}&nbsp;&nbsp;<small>(<a
										href="<c:url value="/stock/stock/detail/${stock.id}"/>">${stock.code}</a>)
								</small></td>
								<!-- 
								...
								<td><span class="pull-left"><fmt:formatNumber
									...
									class="span-suffix pull-right"><fmt:formatNumber
											value="${stock.dailyStat.dynamicPe }" type="number"
											pattern="#,##0.0#" maxFractionDigits="1" groupingUsed="true" />
										TTM</span></td>
								<td><fmt:formatNumber value="${stock.dailyStat.pb }"
										type="number" pattern="#,##0.0#" maxFractionDigits="2"
										groupingUsed="true" /></td>
								<td
									class="text-right ${stock.dailyStat.todayIncreaseRate lt 0 ? "decrease-color":"increase-color"}"><fmt:formatNumber
										value="${stock.dailyStat.todayIncreaseRate *100 }"
										type="number" pattern="#,##0.0#" maxFractionDigits="1"
										groupingUsed="true" />%</td>
								<td
									class="text-right ${stock.dailyStat.priceAmplitude lt 0 ? 'decrease-color':'increase-color'}"><fmt:formatNumber
										value="${stock.dailyStat.priceAmplitude *100 }" type="number"
										pattern="#,##0.0#" maxFractionDigits="1" groupingUsed="true" />%</td>
								<!--@generate-entity-jsp-property-value@-->
								<td><a
									href="<c:url value="/stock/dailyData/home?EQ|stockId=${stock.id}"/>"><fmt:message
											key="stock.dailyData" /></a></td>
								<td><a
									href="<c:url value="/stock/reportData/report?stockId=${stock.id}"/>">财报</a></td>
								<td><a
									href="<c:url value="/stock/starDataStat/home?EQ|stockId=${stock.id}"/>">星标</a></td>
								<td><a
									href="<c:url value="/stock/totalStock/home?EQ|stockId=${stock.id }"/>">股本 </a></td>
								<td><a
									href="<c:url value="/stock/bonusRecord/home?EQ|stockId=${stock.id}"/>">分红</a></td>
								<td><a
									href="<c:url value="/stock/forecast/home?EQ|stockId=${stock.id }"/>">机构估值</a></td>
							<tr>
						</c:forEach>
					</tbody>
				</table>
			</div>
			<f:page page="${page}" url="/stock/stock/home"
