				...
				for(FinancialData profit:netProfit){
					...
					if(profitNum!=null && assetsNum!=null){
						...
						if(dataStat==null){
						    ...
						    if(returnOnAssets.compareTo(dataStat.getReturnOnAssets())==0){
						    	logger.debug(">>FaceYe --> 本次计算的总资产回报率与上次相同，股票："+stock.getName()+"("+stock.getCode()+")");
						    }else{
						    	logger.debug(">>FaceYe --> 两次计算的总资产回报率不同，股票："+stock.getName()+"("+stock.getCode()+")，上次："+dataStat.getReturnOnAssets()+",本次："+returnOnAssets);
						    	dataStat.setReturnOnAssets(returnOnAssets);
						    }
						}
						this.save(dataStat);
